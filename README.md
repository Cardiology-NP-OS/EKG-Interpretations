...
The development-only R-peak harness verifies signed dataset and candidate manifests, spent-dataset exclusions, a restricted full-partition index, and the exact transitive execution bytes before loading signals. It derives patient, exact-waveform, near-duplicate, parent-role, and executable-manifest checks from pseudonymous partition rows; privately loads the signed detector, matcher, scorer, protocol, and policy graph; emits signed, collision-protected bundles; and remains quarantined until separately approved regression policy, baseline, infrastructure, and real development data exist. Every valid launched run now writes an immutable signed start receipt and a separately signed terminal receipt with workflow/retry identity, bounded failure codes, execution state distinct from gate state, and reconciled primary/replay outcomes for eligible, excluded, failed, abstained, and not-run records; an absent terminal receipt remains detectably incomplete. A GitHub-hosted control-plane workflow records each completed source run/retry that it successfully observes, binds the receipt generator to the observer workflow SHA, and emits a deterministic sanitized receipt; exhausted or inconsistent job metadata is recorded as `OBSERVATION_INCOMPLETE`, not misreported as a pre-launch failure. The observer receives no evaluation signing key, corpus, governance mount, or artifact-root access. Its receipt is not an evaluation signature, candidate-execution claim, bundle verification, clinical result, or WORM attestation, and an observer workflow failure remains a monitoring gap. This accounting does not validate the detector. The verifier, evaluated engine, attempt signer, and bundle signer currently share one trusted worker process, so neither the index mount nor signing-key custody is a hard process boundary; access to the governed runner, index, and key remains restricted, and published artifacts are tested not to serialize protected-row identifiers.

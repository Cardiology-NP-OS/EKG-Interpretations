...
The development-only R-peak harness verifies signed manifests, spent-dataset exclusions, and a restricted full-partition index before loading signals. It derives patient, exact-waveform, near-duplicate, parent-role, and executable-manifest checks from pseudonymous partition rows; emits signed, collision-protected bundles; and remains quarantined until separately approved regression policy, baseline, infrastructure, and real development data exist. The verifier and evaluated engine currently share one trusted process, so the index mount is not a hard confidentiality boundary; access to the governed runner and index remains restricted, and published artifacts are tested not to serialize protected-row identifiers.

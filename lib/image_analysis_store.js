...
function validateAnalysis(analysis, caseRecord, extraction) {
  ...
  if (strict) {
    ...
    requireCondition(
      Array.isArray(analysis.implementations) && analysis.implementations.length === expectedFiles.length &&
      analysis.implementations.every((row, index) => row && row.file === expectedFiles[index] && HASH_RE.test(row.sha256)),

...
function validateAnalysis(analysis, caseRecord, extraction) {
  ...
  const strict = extraction.leads.some(lead => lead.quality.tracePolicy === "SINGLE_CONTIGUOUS_DARK_TRACE");
  if (strict) {
    validateConfig(analysis.configuration);
    requireCondition(
      JSON.stringify(analysis.configuration.quality) === JSON.stringify(analysis.qualityPolicy) &&
      analysis.configuration.thresholdAuthority === analysis.thresholdAuthority,
      "IMAGE_ANALYSIS_CONFIG_BINDING",
    );
    verifyAnalysisImplementation(analysis);
  }
  ...
  for (const row of analysis.leadAnalyses) {
    ...
    requireCondition(extractionCandidates.some(lead =>
      JSON.stringify(lead.paperWindow || null) === JSON.stringify(row.paperWindow || null) &&
      JSON.stringify(lead.quality) === JSON.stringify(row.extractionQuality) &&
      (!strict || qualityPass(lead, analysis.qualityPolicy, extraction.calibration))
    ), "IMAGE_ANALYSIS_QUALITY_BINDING");
  ...
  for (const row of analysis.supplementalPaperWindowAnalyses) {
    ...
    requireCondition(row.extractionQuality && typeof row.extractionQuality === "object", "IMAGE_ANALYSIS_QUALITY");
    requireCondition(eligibleSupplemental.some(lead =>
      sourceKey(lead.lead, lead.rhythmStrip, lead.paperWindow) === key &&
      JSON.stringify(lead.quality) === JSON.stringify(row.extractionQuality) &&
      (!strict || qualityPass(lead, analysis.qualityPolicy, extraction.calibration))
    ), "IMAGE_ANALYSIS_QUALITY_BINDING");
    requireCondition(row.measurement && row.measurement.schema === "ekg-waveform-measurement-pipeline-v1", "IMAGE_ANALYSIS_MEASUREMENT");
...
function readAnalysisArtifact(fileOrDir, analysisId) {
  ...
  const body = readStoreFile(artifactFile, MAX_ANALYSIS_BYTES, "IMAGE_ANALYSIS_FILE_REQUIRED");
  const expectedHash = readStoreFile(hashFile, 65, "IMAGE_ANALYSIS_HASH_FILE_REQUIRED").toString("ascii").trim();
  ...
  const artifact = parseStoreJson(body, "IMAGE_ANALYSIS_JSON");
  ...
  return { artifact, base, record, extraction };
}

function readImageAnalysis(fileOrDir, analysisId) {
  const { artifact, base, record, extraction } = readAnalysisArtifact(fileOrDir, analysisId);
  validateAnalysis(base, record, extraction);
  return artifact;
}

function auditImageAnalysis(fileOrDir, analysisId) {
  const { artifact, base, record, extraction } = readAnalysisArtifact(fileOrDir, analysisId);
  requireCondition(base.schema === "ekg-image-signal-analysis-v2", "IMAGE_ANALYSIS_SCHEMA");
  requireCondition(base.caseId === record.caseId, "IMAGE_ANALYSIS_CASE_MISMATCH");
  requireCondition(base.extractionId === extraction.extractionId, "IMAGE_ANALYSIS_EXTRACTION_MISMATCH");
  validateInactiveTree(base);
  return {
    schema: "ekg-image-analysis-audit-v1",
    artifact,
    storedBytesAndSourceIdentityVerified: true,
    implementationProvenance: "NOT_VERIFIED",
...
function persistImageAnalysis(fileOrDir, analysis) {
  ...
  if (fs.existsSync(finalDir)) {
    ...
    return {
      ...
      sha256: artifactHash,
      idempotent: true,
      ...require("./image_signal_analysis").ANALYSIS_GOVERNANCE,
    };
  }

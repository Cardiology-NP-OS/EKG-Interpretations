...
function verifyEvaluationBundle(bundlePath, publicKeyPem, options = {}) {
  ...
  for (const row of rows) {
    ...
    requireCondition(sha256(bytes) === match[1], "EVALUATION_BUNDLE_ARTIFACT_HASH");
    hashes[match[2]] = match[1];
    if (capturedNames.has(match[2])) artifactBytes[match[2]] = bytes;
  }
  ...
  const parsed = {};
  try {
    for (const name of capturedNames) parsed[name] = JSON.parse(artifactBytes[name].toString("utf8"));
  } catch (_) {
    throw new Error("EVALUATION_BUNDLE_ARTIFACT_JSON");
  }
  ...
  return { schema: "ekg-evaluation-bundle-verification-v1", pass: true, runDigest, merkleRootSha256, signerKeyId: signature.signerKeyId, artifactCount: Object.keys(hashes).length, runManifest, artifacts };
}

...
function detectPerspectiveQuadrilateral(image, options = {}) {
  const source = normalizeImage(image);
  const h = source.length;
  const w = source[0].length;
  const darkThreshold = options.darkThreshold === undefined ? 245 : options.darkThreshold;
  const maxSamples = options.maxSamples === undefined ? 250000 : options.maxSamples;
  const minSupportPixels = options.minSupportPixels === undefined ? 500 : options.minSupportPixels;
  const minAreaFraction = options.minAreaFraction === undefined ? 0.10 : options.minAreaFraction;
  const maxAreaFraction = options.maxAreaFraction === undefined ? 0.98 : options.maxAreaFraction;
  ...
  requireCondition(
    Number.isInteger(minSupportPixels) && minSupportPixels >= 100 && minSupportPixels <= maxSamples,
    "IMAGE_PERSPECTIVE_DETECT_MIN_SUPPORT",
  );
  requireCondition(
    typeof minAreaFraction === "number" && Number.isFinite(minAreaFraction) &&
    typeof maxAreaFraction === "number" && Number.isFinite(maxAreaFraction) &&
    minAreaFraction > 0 && minAreaFraction < maxAreaFraction && maxAreaFraction <= 1,
    "IMAGE_PERSPECTIVE_DETECT_AREA_FRACTION",
  );
  ...
  for (let y = 0; y < h; y += 1) {
    for (let x = 0; x < w; x += 1) {
      if (source[y][x] <= darkThreshold) darkCount += 1;
    }
  }
  requireCondition(darkCount >= minSupportPixels, "IMAGE_PERSPECTIVE_DETECT_INSUFFICIENT_SUPPORT");

  const sampleEvery = Math.max(1, Math.ceil(darkCount / maxSamples));
  const points = [];
  let seen = 0;
  for (let y = 0; y < h; y += 1) {
    for (let x = 0; x < w; x += 1) {
      if (source[y][x] > darkThreshold) continue;
      if (seen % sampleEvery === 0) points.push({ x, y });
      seen += 1;
    }
  }
  requireCondition(
    points.length >= minSupportPixels && points.length <= maxSamples,
    "IMAGE_PERSPECTIVE_DETECT_SAMPLE_BUDGET",
  );

  const pick = (score, better, tie) => {
    let best = null;
    let bestScore = null;
    for (const point of points) {
      ...
      if (
        best === null ||
        better(value, bestScore) ||
        (value === bestScore && tie(point, best))
      ) {
        best = point;
        bestScore = value;
      }
  ...
  const minEdge = Math.min(top, bottom, left, right);
  const maxEdge = Math.max(top, bottom, left, right);
  requireCondition(minEdge >= 20 && maxEdge / minEdge <= 8, "IMAGE_PERSPECTIVE_DETECT_EDGE_RATIO");

  const supportFraction = darkCount / (w * h);
  return {
    schema: "ekg-image-perspective-detection-v1",
    corners,
    sampledSupportPoints: points.length,
    totalSupportPixels: darkCount,
    supportFraction,
    quadrilateralArea: area,
    quadrilateralAreaFraction: areaFraction,
    darkThreshold,
    sampleEvery,
    method: "DARK_SUPPORT_DIAGONAL_EXTREMA_V1",
    automatic: true,
    ...GEOMETRY_GOVERNANCE,
  };
}

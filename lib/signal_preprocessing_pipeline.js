...
function runSignalPreprocessingPipeline(input) {
  ...
  return {
    ...
    segments: segmented.segments.map((segment, segmentIndex) => {
      const transformed = applyEngineeringTransformProfile(
        segment.leads,
        contract.targetSamplingRateHz,
        config.engineeringTransformProfile || null
      );
      return {
        segmentIndex,
        ...
        engineeringTransform: transformed.metadata,
        leads: contract.canonicalLeadOrder.map((leadName, leadIndex) => ({
          leadName,
          unit: transformed.metadata ? "standardized" : resampled[leadIndex].unit,
          samples: transformed.leads[leadIndex],
        })),
      };
    }),
    provenance: {
...
function applyEngineeringTransformProfile(leads, sampleRateHz, profileId) {
  if (!profileId) return { leads, metadata: null };
  if (profileId === FOUNDATION_PROFILE_V1.profileId) {
    const result = applyFoundationPretrainingProfile(leads, sampleRateHz);
    return { leads: result.leads, metadata: result.metadata };
  }
  throw new Error("PREPROCESS_ENGINEERING_PROFILE_UNIMPLEMENTED");
}
...
module.exports = {
  PIPELINE_GOVERNANCE,
  applyEngineeringTransformProfile,
  decodePhysicalLeads,

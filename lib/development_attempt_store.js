...
function publishDevelopmentAttemptStart(root, input) {
  ...
  const payload = {
    schema: "ekg-development-attempt-start-v1",
    attemptId,
  ...
  const signed = signedFiles(payload, input.signingPrivateKeyPem, input.signerKeyId, "ekg-development-attempt-start-signature-v1");
...
function verifySignedFile(payloadFile, signatureFile, publicKeyPem, expectedSignerKeyId, expectedSchema) {
  ...
  requireCondition(signature.schema === expectedSchema && signature.algorithm === "Ed25519", "DEVELOPMENT_ATTEMPT_SIGNATURE_METADATA");
  ...
  return { payload: JSON.parse(body.toString("utf8")), payloadSha256: signature.payloadSha256 };
}
...
function verifyDevelopmentAttempt(attemptPath, publicKeyPem, options = {}) {
  ...
  const start = verifySignedFile(path.join(directory, "start.json"), path.join(directory, "start.signature.json"), publicKeyPem, options.expectedSignerKeyId, "ekg-development-attempt-start-signature-v1");
  ...
  if (!fs.existsSync(terminalDir)) return { schema: "ekg-development-attempt-verification-v1", pass: true, attemptId: start.payload.attemptId, executionStatus: "INCOMPLETE", start: start.payload, terminal: null };
  ...
  return { schema: "ekg-development-attempt-verification-v1", pass: true, attemptId: start.payload.attemptId, executionStatus: terminal.payload.executionStatus, start: start.payload, terminal: terminal.payload };
}
...
module.exports = { publishDevelopmentAttemptStart, publishDevelopmentAttemptTerminal, validateAttemptId, verifyCompletedDevelopmentAttempt, verifyDevelopmentAttempt };

...
const { cropImage, rotate90 } = require("./image_robustness");
const { rectifyPerspective, rotateArbitraryInkPreserving } = require("./image_geometry_normalization");
...
function requireStoreDirectory(directory) {
  for (let dir = path.resolve(directory); ; dir = path.dirname(dir)) {
    const info = fs.lstatSync(dir);
    requireCondition(info.isDirectory() && !info.isSymbolicLink(), "IMAGE_STORE_DIRECTORY_REQUIRED");
    if (dir === path.dirname(dir)) break;
  }
}

function readStoreFile(file, maxBytes, code) {
  const resolved = path.resolve(file);
  const before = requireRegularFile(resolved, code);
  requireStoreDirectory(path.dirname(resolved));
  requireCondition(before.size > 0 && before.size <= maxBytes, "IMAGE_STORE_BYTES_LIMIT");
  const identity = stat => [stat.dev, stat.ino, stat.size, stat.mtimeMs, stat.ctimeMs].join(":");
  const fd = fs.openSync(resolved, "r");
  try {
    const opened = fs.fstatSync(fd);
    requireCondition(opened.isFile() && identity(opened) === identity(before), "IMAGE_STORE_FILE_CHANGED");
    ...
    requireCondition(
      offset === opened.size && identity(opened) === identity(fs.fstatSync(fd)) &&
      identity(opened) === identity(requireRegularFile(resolved, code)),
      "IMAGE_STORE_FILE_CHANGED",
    );
...
function parseStoreJson(body, code) {
  let record;
  try { record = JSON.parse(body.toString("utf8")); }
  catch (_) { throw new Error(code); }
  requireCondition(Buffer.from(canonicalJson(record), "utf8").equals(body), code);
  return record;
}
...
function readImageCase(fileOrDir) {
  ...
  const body = readStoreFile(manifest, 32 * 1024 * 1024, "CASE_MANIFEST_REGULAR_FILE_REQUIRED");
  const expected = readStoreFile(hashFile, 65, "CASE_HASH_REGULAR_FILE_REQUIRED").toString("ascii").trim();
  ...
  const record = parseStoreJson(body, "CASE_MANIFEST_JSON");
  ...
  for (const key of ["original", "normalizedRaster"]) {
    ...
    requireCondition(sha256(readStoreFile(artifactPath, MAX_ORIGINAL_BYTES, "CASE_ARTIFACT_REGULAR_FILE_REQUIRED")) === artifact.sha256, "CASE_ARTIFACT_HASH_MISMATCH");
  }
  return record;
}
...
function derivePersistenceArtifacts(intakeInput, intakeResult) {
  ...
  if (geometry && geometry.perspective && geometry.perspective.applied === true) {
    requireCondition(
      geometry.perspective.corners &&
      Number.isInteger(geometry.perspective.outputWidth) &&
      Number.isInteger(geometry.perspective.outputHeight),
      "CASE_PERSPECTIVE_PROVENANCE_REQUIRED",
    );
    normalizedRaster = rectifyPerspective(normalizedRaster, {
      corners: geometry.perspective.corners,
      outputWidth: geometry.perspective.outputWidth,
      outputHeight: geometry.perspective.outputHeight,
    }).image;
  }
  if (geometry && geometry.deskewApplied === true) {
    ...
    normalizedRaster = rotateArbitraryInkPreserving(normalizedRaster, {
      degrees: geometry.deskewCorrectionDegrees,
    });
    if (geometry.crop !== null && geometry.crop !== undefined) {
      requireCondition(
        geometry.crop &&
        Number.isInteger(geometry.crop.x) &&
        Number.isInteger(geometry.crop.y) &&
        Number.isInteger(geometry.crop.width) &&
        Number.isInteger(geometry.crop.height),
        "CASE_DESKEW_CROP_REQUIRED",
      );
      normalizedRaster = cropImage(normalizedRaster, geometry.crop);
    }
  }

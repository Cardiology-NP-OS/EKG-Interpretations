...
function validateRunConfig(config) {
  ...
  const allowed = new Set(["manifestPath", "manifestSignaturePath", "manifestTrustStorePath", "expectedManifestTrustStoreSha256", "partitionIndexPath", "corpusRoot", "artifactRoot", "inputMountMode", "networkIsolation", "storageMode", "startedAtUtc", "trigger", "bootstrap", "previousApprovedBundlePath", "previousBundlePublicKeyPem", "previousBundleSignerKeyId", "signerKeyId", "repositoryRoot", "signingPrivateKeyPem", "allowDirtySyntheticTest", "environmentImageDigest"]);
...
function runDevelopmentEvaluation(config) {
  ...
  const runConfigurationDigest = payloadSha256({ trigger: config.trigger, bootstrap: runtimeConfig.bootstrap || { replicates: protocol.routineBootstrapReplicates, seed: protocol.bootstrapSeed }, networkIsolation: config.networkIsolation, inputMountMode: config.inputMountMode, storageMode: config.storageMode, manifestTrustStoreSha256, previousBaselineIdentity });

...
const { publishDevelopmentAttemptStart, publishDevelopmentAttemptTerminal } = require("./development_attempt_store");
...
function validateRunConfig(config) {
  ...
  const allowed = new Set(["manifestPath", "manifestSignaturePath", "manifestTrustStorePath", "expectedManifestTrustStoreSha256", "partitionIndexPath", "candidateManifestPath", "candidateSignaturePath", "candidateTrustStorePath", "expectedCandidateTrustStoreSha256", "corpusRoot", "artifactRoot", "inputMountMode", "networkIsolation", "storageMode", "startedAtUtc", "trigger", "bootstrap", "previousApprovedBundlePath", "previousBundlePublicKeyPem", "previousBundleSignerKeyId", "signerKeyId", "repositoryRoot", "signingPrivateKeyPem", "allowDirtySyntheticTest", "environmentImageDigest", "attemptId", "workflowRunId", "workflowRunAttempt", "workflowSha"]);
  requireCondition(Object.keys(config).every(key => allowed.has(key)), "DEVELOPMENT_RUN_CONFIG_UNKNOWN_FIELD");
  for (const field of ["manifestPath", "manifestSignaturePath", "candidateManifestPath", "candidateSignaturePath", "candidateTrustStorePath", "expectedCandidateTrustStoreSha256", "corpusRoot", "artifactRoot", "trigger", "signerKeyId", "signingPrivateKeyPem"]) requireCondition(typeof config[field] === "string" && config[field].length > 0, `DEVELOPMENT_RUN_CONFIG_FIELD:${field}`);
...
function executeDevelopmentEvaluation(config, attempt) {
  ...
  const regressionPolicy = execution.regressionPolicy;
  ...
  requireCondition(dirtySyntheticAllowed || identity.commit === config.workflowSha, "DEVELOPMENT_WORKFLOW_SHA_MISMATCH");
  ...
  const states = attempt.stateTransitions;
  const candidateRows = [];
  ...
  for (const manifestRecord of manifest.records.filter(record => record.taskEligibility === "ELIGIBLE")) {
    ...
    recordDetectorOutcome(attempt.accounting, manifestRecord.recordHmacSha256, "CURRENT_ENGINE", "PRIMARY", candidate);
    const candidateReplay = runDetector(candidateDetector, loaded.samples, manifestRecord.sampleRateHz, provenance);
    recordDetectorOutcome(attempt.accounting, manifestRecord.recordHmacSha256, "CURRENT_ENGINE", "REPLAY", candidateReplay);
    const panTompkins = runDetector(panTompkinsDetector, loaded.samples, manifestRecord.sampleRateHz, provenance);
    recordDetectorOutcome(attempt.accounting, manifestRecord.recordHmacSha256, "PAN_TOMPKINS", "PRIMARY", panTompkins);
    const panTompkinsReplay = runDetector(panTompkinsDetector, loaded.samples, manifestRecord.sampleRateHz, provenance);
    recordDetectorOutcome(attempt.accounting, manifestRecord.recordHmacSha256, "PAN_TOMPKINS", "REPLAY", panTompkinsReplay);
  ...
  const current = execution.evaluateRPeakRecords(resultInput(runtimeConfig, manifest, protocol.currentEngine.algorithm, candidateRows, contractDigests));
  const panTompkins = execution.evaluateRPeakRecords(resultInput(runtimeConfig, manifest, execution.panTompkinsAlgorithm, panTompkinsRows, contractDigests));
  transitionAttempt(attempt, "COMPARING");
  const panTompkinsComparison = execution.compareDevelopmentRuns(current, panTompkins, regressionPolicy);
  ...
  if (config.previousApprovedBundlePath) {
    ...
    previousComparison = execution.compareDevelopmentRuns(current, previous.currentEngine, regressionPolicy);
  }
  ...
  const metricCodeDigest = artifactIdentity(["lib/rpeak_development_metrics.js", "lib/event_matcher_v2.js"]);
  ...
  const runConfigurationDigest = payloadSha256({ trigger: config.trigger, bootstrap: runtimeConfig.bootstrap || { replicates: protocol.routineBootstrapReplicates, seed: protocol.bootstrapSeed }, networkIsolation: config.networkIsolation, inputMountMode: config.inputMountMode, storageMode: config.storageMode, manifestTrustStoreSha256, candidateTrustStoreSha256: config.expectedCandidateTrustStoreSha256, candidateManifestPayloadSha256: execution.manifestPayloadSha256, previousBaselineIdentity, workflowRunId: config.workflowRunId, workflowRunAttempt: config.workflowRunAttempt, workflowSha: config.workflowSha });
  ...
  const runIdentityDigest = payloadSha256({
    ...
    matcherDigest: artifactSha256("lib/event_matcher_v2.js"),
  ...
  const accounting = summarizeRunAccounting(attempt.accounting);
  ...
  const runManifest = {
    ...
    executionIdentitySha256: execution.executionIdentitySha256,
    candidateManifestPayloadSha256: execution.manifestPayloadSha256,
    candidateSignerKeyId: execution.signatureVerification.keyId,
    candidateTrustStoreSha256: config.expectedCandidateTrustStoreSha256,
    candidateSignerPublicKeySha256,
  ...
  const artifacts = {
    ...
    "candidate-digests.json": governedArtifact({ currentEngine: current.candidateId, panTompkins: panTompkins.candidateId, candidateDigest, panTompkinsDigest, metricCodeDigest, regressionPolicyDigest, executionIdentity: execution.executionIdentity, executionIdentitySha256: execution.executionIdentitySha256, candidateManifest, candidateSignature, candidateTrustStoreSha256: config.expectedCandidateTrustStoreSha256, candidateSignerKeyId: execution.signatureVerification.keyId, candidateSignerPublicKeySha256, contractDigests }),
    "predictions.restricted.json": governedArtifact({ currentEngine: candidateRows.map(row => ({ recordHmacSha256: row.recordHmacSha256, predictedSampleIndices: row.predictedSampleIndices, status: row.status, failureCode: row.failureCode })), panTompkins: panTompkinsRows.map(row => ({ recordHmacSha256: row.recordHmacSha256, predictedSampleIndices: row.predictedSampleIndices, status: row.status, failureCode: row.failureCode })) }),
    ...
    "internal-result.restricted.json": governedArtifact({ currentEngine: current, panTompkins, engineeringRows, accounting, versusPanTompkins: panTompkinsComparison, versusPreviousApprovedRun: previousComparison }),
    ...
    "engineering-report.json": governedArtifact({ schema: "ekg-development-engineering-report-v1", nRecords: engineeringRows.length, currentEngineTechnicalFailureCount: engineeringRows.filter(row => row.candidate.status !== "SUCCESS").length, panTompkinsTechnicalFailureCount: engineeringRows.filter(row => row.panTompkins.status !== "SUCCESS").length, accounting, preflight }),
    ...
    "logs.json": governedArtifact({ schema: "ekg-development-run-log-v1", attemptId: config.attemptId, executionStatus: "PUBLISHING", gateStatus, stateTransitions: states, accounting }),
  };
...
function runDevelopmentEvaluation(config) {
  ...
  const attempt = {
    stateTransitions: [{ state: "VALIDATING", atUtc: config.startedAtUtc }],
    lastPhase: "VALIDATING",
    accounting: null,
  };
  const handle = publishDevelopmentAttemptStart(config.artifactRoot, {
    attemptId: config.attemptId,
    startedAtUtc: config.startedAtUtc,
    trigger: config.trigger,
    workflowRunId: config.workflowRunId,
  ...
  try {
    const bundle = executeDevelopmentEvaluation(config, attempt);
    ...
    publishDevelopmentAttemptTerminal(handle, {
      ...
      failureCode: null,
      lastPhase,
      stateTransitions: attempt.stateTransitions,
      accounting: attempt.accounting ? summarizeRunAccounting(attempt.accounting) : null,
      bundle: { runId: bundle.runId, runDigest: bundle.runDigest, merkleRootSha256: bundle.merkleRootSha256 },
      signingPrivateKeyPem: config.signingPrivateKeyPem,
      signerKeyId: config.signerKeyId,
    });
    ...
    const lastPhase = attempt.lastPhase;
    const executionStatus = lastPhase === "VALIDATING" ? "BLOCKED" : "FAILED";
    const failedAtUtc = transitionAttempt(attempt, executionStatus);
    publishDevelopmentAttemptTerminal(handle, {
...
module.exports = { assertNoSymlinkPath, deriveContractDigests, implementationDigest, loadEvaluationRecord, normalizeFailureCode, readExact, runDevelopmentEvaluation, runDetector, validateRunConfig };

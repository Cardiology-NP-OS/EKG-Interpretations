...
function runCandidateWorker(config, runtimeIdentitySha256, request, timeoutMs = CANDIDATE_WORKER_TIMEOUT_MS) {
  ...
  const result = childProcess.spawnSync(process.execPath, [worker, config.candidateRoot, config.candidateManifestPath, config.candidateSignaturePath, config.candidateTrustStorePath, config.expectedCandidateTrustStoreSha256, runtimeIdentitySha256, String(config.networkIsolation === "CONTAINER_NETWORK_NONE")], { input: JSON.stringify(request), encoding: "utf8", timeout: timeoutMs, maxBuffer: 64 * 1024 * 1024, windowsHide: true });
...
function executeDevelopmentCandidate(config, executionInput, attempt) {
  ...
  const executeWorker = request => {
    const result = runCandidateWorker(config, runtime.candidateRuntimeIdentitySha256, request, workerBudget.nextWorkerTimeoutMs());
  ...
  for (const record of executionInput.records) {
    ...
    const request = pass => ({ schema: "ekg-development-candidate-prediction-request-v1", requestId: `${record.executionRecordId}:${pass}`, samples: record.samples, sampleRateHz: record.sampleRateHz, configuration: protocol.currentEngine.configuration, provenance });
    const baselineDetector = (samples, sampleRateHz, source) => detectPanTompkinsRPeaks(samples, sampleRateHz, { provenance: source });
    const runs = [
      ["CURRENT_ENGINE", "PRIMARY", executeWorker(request("PRIMARY"))],
      ["CURRENT_ENGINE", "REPLAY", executeWorker(request("REPLAY"))],
      ["PAN_TOMPKINS", "PRIMARY", runDetector(baselineDetector, record.samples, record.sampleRateHz, provenance)],
      ["PAN_TOMPKINS", "REPLAY", runDetector(baselineDetector, record.samples, record.sampleRateHz, provenance)],
    ];
...
function runDevelopmentCandidateExecution(config) {
  ...
  try {
    ...
    executeDevelopmentCandidate(config, executionInput, attempt);
...
module.exports = { assertNoSymlinkPath, createCandidateWorkerBudget, deriveContractDigests, gitIdentity, implementationDigest, loadDevelopmentGovernance, loadEvaluationRecord, normalizeFailureCode, readExact, readJson, readJsonWithSha256, requirePathWithin, runConfigurationIdentity, runDevelopmentCandidateExecution, runDevelopmentEvaluation, runDetector, transitionAttempt, utcNow, validateCandidateRunConfig, validateRunConfig };

...
const { deskewImage, rectifyPerspective } = require("./image_geometry_normalization");
...
function runImageIntakePipeline(input) {
  ...
  const perspectiveRequested = input.perspectiveCorners !== undefined && input.perspectiveCorners !== null;
  requireCondition(
    !(suppliedRoisAtInput && perspectiveRequested),
    "INTAKE_PERSPECTIVE_WITH_EXPLICIT_ROIS_UNSUPPORTED",
  );
  requireCondition(
    !perspectiveRequested || input.perspectiveCornersVerified === true,
  ...
  if (perspectiveRequested) {
    perspective = rectifyPerspective(sourceRaster, {
      corners: input.perspectiveCorners,

...
const { deskewImage } = require("./image_geometry_normalization");
...
function runImageIntakePipeline(input) {
  ...
  if (input.allowDeskewSearch === true) {
    deskew = deskewImage(sourceRaster, {
  ...
  const explicitVoltageCalibration =
    ...
    digitized.leads.every(lead =>
      lead.quality && lead.quality.baselineSource === "EXPLICIT_ROI"
    );
  ...
  const report = {
    ...
    geometryNormalization: deskew ? {
      deskewApplied: deskew.appliedDegrees !== 0,
      deskewCorrectionDegrees: deskew.appliedDegrees,
      deskewEstimatedCorrectionDegrees: deskew.estimate.correctionDegrees,
      deskewScoreGain: deskew.estimate.scoreGain,
      method: deskew.method,
    } : {
      deskewApplied: false,
      deskewCorrectionDegrees: 0,
      deskewEstimatedCorrectionDegrees: 0,
      deskewScoreGain: 0,
      method: "NONE",
    },
    orientationTurns,
    ...
    analysisPermissions: preflight ? {
      ...
      exactTimeMeasurementAllowed: sourceTimePermission,
      exactVoltageMeasurementAllowed: sourceVoltagePermission,
      specificLeadClaimsAllowed: suppliedRois,
      twelveLeadClaimsAllowed: suppliedRois && rois.completeTwelveLeadPanels === true,

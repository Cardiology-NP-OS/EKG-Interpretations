...
function panelBox(g, row, col, cols) {
  ...
  const panelW = Math.round(duration * g.paperSpeedMmPerS * px);
  const fullW = Math.round(g.rhythmDurationS * g.paperSpeedMmPerS * px);
  const x = margin + (row === 3 ? 0 : col * panelW);
  const y = margin + row * rowH;
  const width = row === 3 ? fullW : panelW;

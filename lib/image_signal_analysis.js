...
function runImageSignalAnalysis(extraction, config) {
  ...
  return {
    ...
    diagnosticInterpretationIncluded: false,
    ...(extraction.leads.some(lead => lead.quality.tracePolicy === "SINGLE_CONTIGUOUS_DARK_TRACE") ? {
      ...
      implementations: [
        ...
        file: `lib/${name}.js`,
        sha256: crypto.createHash("sha256").update(fs.readFileSync(require.resolve(`./${name}`))).digest("hex"),
      })),
    } : {}),
    ...ANALYSIS_GOVERNANCE,
  };

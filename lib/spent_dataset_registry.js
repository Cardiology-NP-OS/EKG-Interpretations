...
const MINIMUM_SPENT_REGISTRY_SEQUENCE = 1;
const PINNED_SPENT_REGISTRY_SHA256 = "9343816ec033a411998d2aad3930ed9de1ca2e136e5bdbd7d2c67efcf5c61fdf";
...
function identityTokens(identity) {
  ...
  for (const value of [identity.benchmarkId, identity.datasetId, identity.datasetFamily, identity.datasetName, identity.canonicalSource, ...aliases]) {
...
function identityHashes(identity) {
  ...
  const contentHashes = Array.isArray(identity.contentSha256) ? identity.contentSha256 : [];
  for (const value of [identity.archiveSha256, identity.sourceManifestSha256, identity.recordSetSha256, identity.patientSetSha256, ...contentHashes]) {
    if (value !== undefined && value !== null) hashes.add(validateSha256(value, "SPENT_REGISTRY_IDENTITY_HASH"));
  }
  return hashes;
}

function validateIdentityShape(identity, code) {
  requireCondition(plain(identity), `${code}_OBJECT`);
  requireCondition(identity.aliases === undefined || Array.isArray(identity.aliases) && identity.aliases.every(value => typeof value === "string" && value.trim().length > 0), `${code}_ALIASES`);
  requireCondition(identity.contentSha256 === undefined || Array.isArray(identity.contentSha256), `${code}_CONTENT_HASHES`);
}
...
function validateEntry(entry) {
  ...
  requireCondition(Array.isArray(entry.aliases) && entry.aliases.length > 0 && entry.aliases.every(value => typeof value === "string" && value.trim().length > 0), "SPENT_REGISTRY_ALIASES");
  ...
  entry.deniedIdentities.forEach(identity => {
    validateIdentityShape(identity, "SPENT_REGISTRY_IDENTITY");
...
function assertDevelopmentIdentityAllowed(requested, registry) {
  validateIdentityShape(requested, "SPENT_REGISTRY_REQUEST");
...
module.exports = { MINIMUM_SPENT_REGISTRY_SEQUENCE, PINNED_SPENT_REGISTRY_SHA256, assertDevelopmentIdentityAllowed, matchSpentIdentity, validateSpentRegistry };

...
const PHASES = Object.freeze(["SYNTHETIC_BOUNDARY_NOT_PASSED", "FULL_DEVELOPMENT_NOT_STARTED", "GOVERNED_RESOURCE_PREFLIGHT_NOT_PASSED", "SIGNING_KEY_NOT_PREPARED", "RUN_CONFIGURATION_NOT_CREATED", "RUN_CONFIGURATION_STEP_COMPLETED", "EVALUATION_STEP_REACHED", "SOURCE_WORKFLOW_COMPLETED", "OBSERVATION_INCOMPLETE"]);
...
function validateEvidenceLifecycle(sourceConclusion, evidence) {
  const executionSteps = ["governedResourcePreflight", "signingKeyPreparation", "runConfigurationStep", "evaluationStep"];
  if (evidence.syntheticBoundaryConclusion === "NOT_OBSERVED") {
    ...
    if (evidence.governedResourcePreflight !== "PASSED") requireCondition(["signingKeyPreparation", "runConfigurationStep", "evaluationStep"].every(field => evidence[field] === "NOT_OBSERVED"), "DEVELOPMENT_WORKFLOW_RECEIPT_EVIDENCE_ORDER");
    if (evidence.signingKeyPreparation !== "PASSED") requireCondition(["runConfigurationStep", "evaluationStep"].every(field => evidence[field] === "NOT_OBSERVED"), "DEVELOPMENT_WORKFLOW_RECEIPT_EVIDENCE_ORDER");
    if (evidence.runConfigurationStep !== "PASSED") requireCondition(evidence.evaluationStep === "NOT_OBSERVED", "DEVELOPMENT_WORKFLOW_RECEIPT_EVIDENCE_ORDER");
  }
...
function derivePhase(sourceConclusion, evidence) {
  ...
  if (evidence.evaluationStep === "NOT_OBSERVED") return "RUN_CONFIGURATION_STEP_COMPLETED";
...
function validateDevelopmentWorkflowAttemptReceipt(receipt) {
  ...
  const evidenceFields = ["syntheticBoundaryConclusion", "fullDevelopmentConclusion", "governedResourcePreflight", "signingKeyPreparation", "runConfigurationStep", "evaluationStep", "reconciliationStep", "cleanupStep"];

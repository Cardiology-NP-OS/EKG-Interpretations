...
function patientClusterBootstrap(records, options = {}) {
  ...
  const values = { sensitivity: [], ppv: [], f1: [] };
  ...
  for (let replicate = 0; replicate < replicates; replicate += 1) {
    ...
    for (const name of Object.keys(values)) if (metrics[name] !== null) values[name].push(metrics[name]);
...
module.exports = { RPEAK_METRIC_VERSION, aggregateByPatient, countsMetrics, distribution, evaluateRPeakRecords, patientClusterBootstrap, quantile, scoreRecord };

...
function walkFiniteNumbers(value, path, errors) {
  if (typeof value === "number" && !Number.isFinite(value)) {
    errors.push(`NONFINITE_NUMERIC_VALUE:${path}`);
    return;
  }
  if (Array.isArray(value)) {
    value.forEach((item, index) => walkFiniteNumbers(item, `${path}[${index}]`, errors));
    return;
  }
  if (isPlainObject(value)) {
    for (const [key, item] of Object.entries(value)) {
      walkFiniteNumbers(item, path ? `${path}.${key}` : key, errors);
    }
  }
}
...
function preflightInput(input) {
  ...
  const scaleTrustworthy = !geometryDistorted || input.calibration.local_scale_trustworthy;
  ...
  if (input.embedded_text.some(text => typeof text === "string" && instructionLikeText(text))) {
    pushUnique(warnings, "POSSIBLE_PROMPT_INJECTION");
  }
  if (input.embedded_text.some(text => typeof text !== "string")) {
    return malformedResult(["EMBEDDED_TEXT_ITEM"]);
  }

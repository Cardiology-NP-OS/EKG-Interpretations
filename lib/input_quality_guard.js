...
const SUPPORTED_FORMATS = new Set(["pdf", "png", "jpeg", "jpg", "raster_matrix", "structured_json"]);
...
const STRUCTURE_LIMITS = Object.freeze({
  ...
  maxNodes: 10000,
  maxStringChars: 131072,
  maxTotalStringChars: 1048576,
});

const UNTRUSTED_TEXT_FIELDS = Object.freeze([
  "embedded_text",
  "ocr_text",
  "qr_text",
  "metadata_text",
  "machine_interpretation_text",
]);

const TEXT_LIMITS = Object.freeze({
  maxItemsPerChannel: 256,
  maxItemChars: 65536,
  maxTotalChars: 262144,
  maxFilenameChars: 4096,
});
...
function walkFiniteNumbers(
  ...
  errors,
  active = new WeakSet(),
  state = { nodes: 0, stringChars: 0, limited: false },
  depth = 0
) {
  ...
  if (
    ...
    errors.push(`UNSUPPORTED_PROXY_OBJECT:${path}`);
    return;
  }
  ...
  if (type === "string") {
    ...
    if (value.length > STRUCTURE_LIMITS.maxStringChars) {
      errors.push(`STRUCTURE_STRING_LENGTH:${path}`);
      state.limited = true;
      return;
    }
    if (state.stringChars > STRUCTURE_LIMITS.maxTotalStringChars) {
      errors.push(`STRUCTURE_STRING_BUDGET:${path}`);
      state.limited = true;
      return;
    }
    return;
  }
  if (value === null || ["boolean", "number"].includes(type)) return;
...
function normalizeIdentifier(text) {
  return normalizeUntrustedText(text).toLowerCase();
}
...
function malformedResult(reasons) {
  return {
    ...
    sourceBinding: {
      ...
      format,
      imageWidthPx: input.image.width_px,
      imageHeightPx: input.image.height_px,
      paperSpeedMmPerS: speed,
      gainMmPerMv: gain,
      leadLabelsVerified: input.lead_labels_verified,
      normalizedLeadLabels: [...new Set(normalizedLeads)],
    },
...
function preflightInput(input) {
  ...
  if (input !== null && typeof input === "object" && isProxy(input)) {
    return malformedResult(["UNSUPPORTED_PROXY_OBJECT:"]);
  }
  ...
  if (
    Array.isArray(input.lead_labels) &&
    input.lead_labels.some(label => typeof label !== "string")
  ) {
    malformed.push("LEAD_LABEL_ITEM");
  }
  ...
  for (const field of UNTRUSTED_TEXT_FIELDS.slice(1)) {
  ...
  let totalUntrustedTextChars =
    typeof input.filename === "string" ? input.filename.length : 0;
  if (
    typeof input.filename === "string" &&
    input.filename.length > TEXT_LIMITS.maxFilenameChars
  ) {
    malformed.push("FILENAME_LENGTH_LIMIT");
  }
  for (const field of UNTRUSTED_TEXT_FIELDS) {
    const values = input[field] || [];
    if (values.length > TEXT_LIMITS.maxItemsPerChannel) {
      malformed.push(`TEXT_CHANNEL_ITEM_LIMIT:${field}`);
      continue;
    }
    for (const value of values) {
      ...
      if (value.length > TEXT_LIMITS.maxItemChars) {
        malformed.push(`TEXT_ITEM_LENGTH_LIMIT:${field}`);
      }
      totalUntrustedTextChars += value.length;
    }
  }
  if (totalUntrustedTextChars > TEXT_LIMITS.maxTotalChars) {
    malformed.push("TEXT_TOTAL_LENGTH_LIMIT");
  }
  if (malformed.length) return malformedResult(malformed);
  ...
  const transformedScale =
    geometryDistorted ||
    input.quality_flags.includes("resampled") ||
    input.quality_flags.includes("cropped");
  ...
  for (const claim of input.metadata_claims) {
    ...
    const key = normalizeIdentifier(claim.field);
    if (!key) {
      malformed.push("METADATA_CLAIM");
      continue;
    }
  ...
  for (const measurement of input.measurements) {
    ...
    const normalizedName = normalizeIdentifier(measurement.name);
    const normalizedUnit = normalizeIdentifier(measurement.unit);

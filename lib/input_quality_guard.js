...
function walkFiniteNumbers(value, path, errors, seen = new WeakSet()) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      walkFiniteNumbers(item, `${path}[${index}]`, errors, seen)
    );
    return;
  }
  if (isPlainObject(value)) {
    for (const [key, item] of Object.entries(value)) {
      walkFiniteNumbers(item, path ? `${path}.${key}` : key, errors, seen);
    }
  }
}
...
function preflightInput(input) {
  ...
  for (const field of ["multiple_tracings_present", "tracing_identity_match_established"]) {

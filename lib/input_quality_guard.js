...
function walkFiniteNumbers(value, path, errors, active = new WeakSet()) {
  ...
  const traversable = Array.isArray(value) || isPlainObject(value);
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      walkFiniteNumbers(item, `${path}[${index}]`, errors, active)
    );
  } else {
    for (const [key, item] of Object.entries(value)) {
      walkFiniteNumbers(item, path ? `${path}.${key}` : key, errors, active);
    }
  }
  active.delete(value);
}

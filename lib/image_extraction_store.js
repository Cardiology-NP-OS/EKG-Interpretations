...
const { CASE_GOVERNANCE, readImageCase, readStoreFile, parseStoreJson, requireStoreDirectory } = require("./image_case_store");
...
function readImageExtraction(fileOrDir, extractionId) {
  ...
  const body = readStoreFile(artifactFile, MAX_EXTRACTION_BYTES, "EXTRACTION_FILE_REQUIRED");
  const expected = readStoreFile(hashFile, 65, "EXTRACTION_HASH_FILE_REQUIRED").toString("ascii").trim();
  ...
  const artifact = parseStoreJson(body, "EXTRACTION_JSON");

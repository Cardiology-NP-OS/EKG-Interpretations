...
const GATE_FIELDS = Object.freeze(["id", "metricPath", "direction", "absoluteFloor", "noninferiorityMargin", "ciRule", "minimumDenominator", "denominatorPath", "blocking"]);
const METRIC_SEMANTICS = Object.freeze({
  "summary.micro.sensitivity": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.micro.ppv": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.micro.f1": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.micro.falseDetectionsPerHour": Object.freeze({ direction: "lower", domain: "NONNEGATIVE" }),
  "summary.recordMacro.sensitivity.p05": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.recordMacro.sensitivity.minimum": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.recordMacro.sensitivity.p50": Object.freeze({ direction: "higher", domain: "PROPORTION" }),
  "summary.timing.absoluteError.p95": Object.freeze({ direction: "lower", domain: "NONNEGATIVE" }),
  "summary.technicalFailureRate": Object.freeze({ direction: "lower", domain: "PROPORTION" }),
});
...
function validatePolicy(policy) {
  ...
  for (const gate of gates) {
    ...
    requireCondition(Object.getOwnPropertySymbols(gate).length === 0 && Object.getOwnPropertyNames(gate).every(field => GATE_FIELDS.includes(field)), "RUN_COMPARISON_GATE_FIELDS");
    ...
    const semantics = METRIC_SEMANTICS[gate.metricPath];
    requireCondition(gate.direction === semantics.direction, "RUN_COMPARISON_GATE_DIRECTION");
    ...
    if (gate.absoluteFloor !== undefined) {
      if (semantics.domain === "PROPORTION") requireCondition(gate.absoluteFloor >= 0 && gate.absoluteFloor <= 1, "RUN_COMPARISON_GATE_ABSOLUTE_FLOOR_DOMAIN");
      else requireCondition(gate.absoluteFloor >= 0, "RUN_COMPARISON_GATE_ABSOLUTE_FLOOR_DOMAIN");
      requireCondition(semantics.domain !== "PROPORTION" || (semantics.direction === "higher" ? gate.absoluteFloor > 0 : gate.absoluteFloor < 1), "RUN_COMPARISON_GATE_ABSOLUTE_FLOOR_VACUOUS");
    }
    if (gate.noninferiorityMargin !== undefined && semantics.domain === "PROPORTION") {
      requireCondition(gate.noninferiorityMargin <= 1, "RUN_COMPARISON_GATE_MARGIN_DOMAIN");
      requireCondition(gate.noninferiorityMargin < 1, "RUN_COMPARISON_GATE_MARGIN_VACUOUS");
    }
...
module.exports = { assertComparable, compareDevelopmentRuns, metricValue, pairedBootstrap, validatePolicy };

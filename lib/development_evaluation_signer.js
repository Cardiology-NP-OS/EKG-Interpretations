...
function loadSignerControls(repositoryRoot, execution, budget) {
  const controls = {};
  for (const [name, file] of Object.entries(SIGNER_CONTROL_FILES)) {
    ...
    const snapshot = readDevelopmentControlJson(path.join(repositoryRoot, file), DEVELOPMENT_CONTROL_RESOURCE_LIMITS.maxSignerControlBytes, budget, "DEVELOPMENT_SIGNER_CONTROL_ARTIFACT_JSON");
    requireCondition(snapshot.bytes.length === candidateArtifact.identity.bytes && snapshot.sha256 === candidateArtifact.identity.sha256, "DEVELOPMENT_SIGNER_CONTROL_ARTIFACT_MISMATCH");
    requireCondition(Buffer.from(snapshot.bytes.toString("utf8"), "utf8").equals(snapshot.bytes), "DEVELOPMENT_SIGNER_CONTROL_ARTIFACT_ENCODING");
    controls[name] = snapshot.value;
  }
...
function prepareSignerInputs(config, options = {}) {
  ...
  const controlBudget = options.controlBudget || createDevelopmentControlBudget();
  ...
  const candidateManifest = readJson(config.candidateManifestPath, "DEVELOPMENT_CANDIDATE_MANIFEST_JSON", { maxBytes: DEVELOPMENT_CONTROL_RESOURCE_LIMITS.maxManifestBytes, budget: controlBudget });
  const candidateSignature = readJson(config.candidateSignaturePath, "DEVELOPMENT_CANDIDATE_SIGNATURE_JSON", { maxBytes: DEVELOPMENT_CONTROL_RESOURCE_LIMITS.maxSignatureBytes, budget: controlBudget });
  const candidateTrustStore = readJsonWithSha256(config.candidateTrustStorePath, config.expectedCandidateTrustStoreSha256, "DEVELOPMENT_CANDIDATE_TRUST_JSON", { maxBytes: DEVELOPMENT_CONTROL_RESOURCE_LIMITS.maxTrustStoreBytes, budget: controlBudget });
  ...
  const signerControls = loadSignerControls(SIGNER_ROOT, execution, controlBudget);
  ...
  const governance = loadDevelopmentGovernance(config, repositoryRoot, syntheticTest, { controlBudget });
  ...
  return { repositoryRoot, candidateRoot, syntheticTest, candidateManifest, candidateSignature, candidateTrustStore, execution, signerControls, candidateRuntime, candidateIsolation, governance, identity, loadedRecords, executionInput, runBinding, verifiedControlBytes: controlBudget.totalBytes, controlResourceLimits: DEVELOPMENT_CONTROL_RESOURCE_LIMITS };
}

function startDevelopmentEvaluationAttempt(config, signingPrivateKeyPem, options = {}) {
  validateRunConfig(config);
  signerPublicKey(signingPrivateKeyPem);
  const prepared = prepareSignerInputs(config, { ...options, createExecutionInput: true });
...
function loadSignerContext(config, handoff, options = {}) {
  requireCondition(process.env.EKG_EVALUATION_NETWORK_DISABLED === "1", "DEVELOPMENT_NETWORK_ISOLATION_ENV_REQUIRED");
  requireCondition(["CONTAINER_NETWORK_NONE", "SYNTHETIC_TEST_PROCESS"].includes(config.networkIsolation), "DEVELOPMENT_NETWORK_ISOLATION_REQUIRED");
  requireCondition(config.inputMountMode === "READ_ONLY", "DEVELOPMENT_INPUT_READ_ONLY_REQUIRED");
  requireCondition(config.storageMode === "APPLICATION_WRITE_ONCE_SIGNED", "DEVELOPMENT_STORAGE_MODE");
  ...
  const prepared = prepareSignerInputs(config, options);
...
function buildEvaluationBundle(config, handoff, context, attempt, signingPrivateKeyPem) {
  ...
  if (config.previousApprovedBundlePath) {
    ...
    const priorVerification = verifyEvaluationBundle(previousApprovedBundlePath, config.previousBundlePublicKeyPem, { expectedSignerKeyId: config.previousBundleSignerKeyId, requestedArtifactNames: ["internal-result.restricted.json"] });
    ...
    const previous = priorVerification.artifacts["internal-result.restricted.json"];
    requireCondition(previous && typeof previous === "object" && !Array.isArray(previous), "DEVELOPMENT_PREVIOUS_RESULT_JSON");
  ...
  const harnessCodeDigest = implementationDigest([
    path.join(repositoryRoot, "lib", "development_candidate_worker.js"),
    path.join(repositoryRoot, "lib", "development_control_snapshot.js"),
  ...
  try {
    ...
    return { schema: "ekg-evaluation-bundle-receipt-v1", runId, path: existingPath, runDigest: existing.runDigest, merkleRootSha256: existing.merkleRootSha256, gateStatus, recoveredExistingBundle: true, runManifest: existing.runManifest, writeOnce: false, applicationCollisionProtected: true, externalObjectLockAttested: false, storageMode: config.storageMode, bundleResourceLimits: existing.bundleResourceLimits, clinicalAccuracyClaimed: false, capabilityNotClaim: true, reportable: false, runtimeAuthority: false, projectGold: false, sourceLabelsAreProjectGold: false };
  }
}
...
function finalizeDevelopmentEvaluationAttempt(config, handoffSource, signingPrivateKeyPem, options = {}) {
  validateRunConfig(config);
  ...
  try {
    ...
    const context = loadSignerContext(config, handoff, options);

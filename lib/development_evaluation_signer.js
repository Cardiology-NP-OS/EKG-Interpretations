...
const { verifyDevelopmentExecutionPackage } = require("./development_execution_identity");
const { readDevelopmentExecutionHandoff, validateDevelopmentExecutionHandoff } = require("./development_execution_handoff");
...
const { PAN_TOMPKINS_ALGORITHM } = require("./pan_tompkins_detector");
const { evaluateRPeakRecords } = require("./rpeak_development_metrics");
const { payloadSha256 } = require("./evaluation_signatures");
const { deriveContractDigests, implementationDigest, loadDevelopmentGovernance, loadEvaluationRecord, readJson, readJsonWithSha256, requirePathWithin, runConfigurationIdentity, transitionAttempt, utcNow, validateRunConfig } = require("./development_evaluation_runner");
...
function prepareSignerInputs(config) {
  ...
  const execution = verifyDevelopmentExecutionPackage(candidateManifest, candidateSignature, candidateTrustStore, { repositoryRoot: candidateRoot });
  const governance = loadDevelopmentGovernance(config, repositoryRoot, syntheticTest);
  const dirtySyntheticAllowed = config.allowDirtySyntheticTest === true && syntheticTest && governance.manifest.datasetIdentity.datasetId === "ECG-DATASET-SYNTHETIC-DEVELOPMENT";
  ...
  return { repositoryRoot, candidateRoot, syntheticTest, candidateManifest, candidateSignature, candidateTrustStore, execution, governance, identity, runBinding };
}
...
function loadSignerContext(config, handoff) {
  ...
  const prepared = prepareSignerInputs(config);
  const { repositoryRoot, syntheticTest, candidateManifest, candidateSignature, candidateTrustStore, execution, governance, identity } = prepared;
  ...
  const expectedHandoff = {
    ...
    sampleCounts: new Map(manifest.records.filter(record => record.taskEligibility === "ELIGIBLE").map(record => [record.recordHmacSha256, Math.round(record.durationSeconds * record.sampleRateHz)])),
  };
  validateDevelopmentExecutionHandoff(handoff, manifest, expectedHandoff);
  ...
  if (handoff.handoffStatus === "EXECUTED") {
    ...
    validateDevelopmentExecutionHandoff(handoff, manifest, { ...expectedHandoff, sampleCounts: new Map([...loadedRecords].map(([record, loaded]) => [record, loaded.samples.length])) });
  }
  return { repositoryRoot, candidateManifest, candidateSignature, candidateTrustStore, execution, protocol, regressionPolicy, governance, manifest, identity, loadedRecords, runBinding: prepared.runBinding };
}

function reconstructExecution(manifest, loadedRecords, handoff, protocol, panTompkinsAlgorithm) {
  const accounting = createRunAccounting(manifest);
  for (const row of handoff.outcomes) recordDetectorOutcome(accounting, row.recordHmacSha256, row.detector, row.pass, row);
  if (handoff.handoffStatus !== "EXECUTED") return { accounting, candidateRows: [], panTompkinsRows: [], engineeringRows: [] };
  const byIdentity = new Map(handoff.outcomes.map(row => [`${row.recordHmacSha256}:${row.detector}:${row.pass}`, row]));
  const candidateRows = [];
  ...
  for (const manifestRecord of manifest.records.filter(record => record.taskEligibility === "ELIGIBLE")) {
    const key = (detector, pass) => byIdentity.get(`${manifestRecord.recordHmacSha256}:${detector}:${pass}`);
    const candidate = key("CURRENT_ENGINE", "PRIMARY");
    const candidateReplay = key("CURRENT_ENGINE", "REPLAY");
    const panTompkins = key("PAN_TOMPKINS", "PRIMARY");
    const panTompkinsReplay = key("PAN_TOMPKINS", "REPLAY");
...
function buildEvaluationBundle(config, handoff, context, attempt, signingPrivateKeyPem) {
  ...
  const { repositoryRoot, candidateManifest, candidateSignature, candidateTrustStore, execution, protocol, regressionPolicy, governance, manifest, identity, loadedRecords } = context;
  const reconstructed = reconstructExecution(manifest, loadedRecords, handoff, protocol, PAN_TOMPKINS_ALGORITHM);
  ...
  const candidateReferenceIsolation = false;
  ...
  const artifacts = {
    ...
    "gates.json": governedArtifact({ schema: "ekg-development-gates-v1", status: gateStatus, versusPanTompkins: panTompkinsComparison.status, versusPreviousApprovedRun: previousComparison.status, regressionPolicyApproval: regressionPolicy.approvalStatus, policyReady, candidateReferenceIsolation, referenceIsolationRequiredForPass: true }),
...
function finalizeDevelopmentEvaluationAttempt(config, handoffSource, signingPrivateKeyPem) {
  ...
  try {
    ...
    const reconstructed = reconstructExecution(context.manifest, context.loadedRecords, handoff, context.protocol, PAN_TOMPKINS_ALGORITHM);
...
module.exports = { finalizeDevelopmentEvaluationAttempt, prepareSignerInputs, reconstructExecution, startDevelopmentEvaluationAttempt };

...
const { verifyDevelopmentPartition } = require("./development_partition_verifier");
...
function validatePartitionAttestation(attestation) {
  ...
  if (attestation.roleRecordCounts !== undefined) {
    requireCondition(plain(attestation.roleRecordCounts), "DEVELOPMENT_PARTITION_ROLE_COUNTS");
    exactKeys(attestation.roleRecordCounts, ["development", "selection", "lockedHoldout"], "DEVELOPMENT_PARTITION_ROLE_COUNTS_FIELDS");
    requireCondition(Object.values(attestation.roleRecordCounts).every(value => Number.isInteger(value) && value > 0), "DEVELOPMENT_PARTITION_ROLE_COUNTS_VALUE");
  }
...
function computedManifestIdentity(manifest, partitionIndex) {
  const eligible = manifest.records.filter(record => record.taskEligibility === "ELIGIBLE");
  const partitionContent = partitionIndex && Array.isArray(partitionIndex.rows) ? partitionIndex.rows.map(record => record.sourceFileSha256) : [];
  return {
    ...
    contentSha256: [...eligible.flatMap(record => [record.sourceFileSha256, record.labelSnapshotSha256]), ...partitionContent],
  };
}
...
function preflightDevelopmentRun(input) {
  ...
  const partitionVerification = verifyDevelopmentPartition(input.partitionIndex, input.manifest);
  assertDevelopmentIdentityAllowed(computedManifestIdentity(input.manifest, input.partitionIndex), input.spentRegistry);
  return {
    ...
    partition: partitionVerification,
    ...CLAIM_BOUNDARY,
  };
}

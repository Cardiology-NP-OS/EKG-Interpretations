...
const { detectCandidateRPeaksV2, QRS_V2_ALGORITHM } = require("./qrs_detector_v2");
...
function runPhysicalLeadMeasurementPipeline(input) {
  ...
  const rPeaks = config.detector.algorithm === QRS_V2_ALGORITHM
    ? detectCandidateRPeaksV2(lead.samples, lead.sampleRateHz, {
      configuration: config.detector.configuration,
      provenance: input.provenance,
    })
    : detectCandidateRPeaks(lead.samples, lead.sampleRateHz, {
      ...config.detector,
      provenance: input.provenance,
    });

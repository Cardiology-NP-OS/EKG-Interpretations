...
assert.match(signerDockerfile, /COPY evaluation\/protocols\/DEVELOPMENT_RPEAK_EVALUATION_V1\.json \.\/evaluation\/protocols\/DEVELOPMENT_RPEAK_EVALUATION_V1\.json/);
assert.match(signerDockerfile, /COPY evaluation\/protocols\/DEVELOPMENT_RPEAK_REGRESSION_POLICY_V1\.json \.\/evaluation\/protocols\/DEVELOPMENT_RPEAK_REGRESSION_POLICY_V1\.json/);
assert.equal(signerDockerfile.includes("COPY evaluation/protocols ./evaluation/protocols"), false);
...
assert.equal((candidateRuntime.match(/executeWorker\(request\("/g) || []).length, 4);
assert.equal(candidateRuntime.includes("runDetector("), false);
assert.equal(candidateRuntime.includes("require(\"./pan_tompkins_detector\")"), false);
assert.equal(candidateRuntime.includes("execution.detectCandidateRPeaks"), false);
assert.match(candidateWorker, /request\.detector === "CURRENT_ENGINE" \? execution\.detectCandidateRPeaks : execution\.detectPanTompkinsRPeaks/);

...
test("one-pixel box blur fails closed at the unchanged ink-band discovery threshold", () => {
  const fx = labeledFixture();
  const degraded = boxBlur(fx.image, { radius: 1 });
  assert.throws(
    () => runAutomatic(degraded, "case://degradation-box-blur-1"),
    error => error.message === "DISCOVERY_NO_INK_BANDS",
  );
  console.log(`BOX_BLUR_PROFILE ${JSON.stringify({ stage: "FAIL_CLOSED", reason: "DISCOVERY_NO_INK_BANDS" })}`);
});
...
console.log(JSON.stringify({
  ...
  profiles: {
    ...
    failsClosed: ["box-blur-radius-1-ink-band-discovery", "v1-label-occlusion", "v4-six-column-trace-occlusion"],
  },
  syntheticOnly: true,

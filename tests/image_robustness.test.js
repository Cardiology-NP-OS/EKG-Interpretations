...
test("noise field rejects unsafe dimensions and allocation budgets before generation",()=>{
  for(const value of [0,-1,0.5,NaN,Infinity,Number.MAX_SAFE_INTEGER+1,"4"]){
    assert.throws(()=>deterministicNoiseField(value,1),/IMAGE_ROBUSTNESS_NOISE_DIMENSIONS/);
    assert.throws(()=>deterministicNoiseField(1,value),/IMAGE_ROBUSTNESS_NOISE_DIMENSIONS/);
  }
  for(const [width,height] of [[4001,1],[1,4001],[2001,2000],[Number.MAX_SAFE_INTEGER,1]]){
    assert.throws(()=>deterministicNoiseField(width,height),/IMAGE_ROBUSTNESS_NOISE_PIXEL_BUDGET/);
  }
  assert.deepStrictEqual(deterministicNoiseField(2,2,{amplitude:0}),[[0,0],[0,0]]);
  for(const period of [3,4,250,251]){
    assert.ok(deterministicNoiseField(32,16,{amplitude:64,period}).flat().every(v=>Number.isFinite(v)&&Math.abs(v)<=64));
  }
  for(const amplitude of [-1,65,0.5,NaN])assert.throws(()=>deterministicNoiseField(1,1,{amplitude}),/IMAGE_ROBUSTNESS_NOISE_AMPLITUDE/);
  for(const period of [2,252,3.5,Infinity])assert.throws(()=>deterministicNoiseField(1,1,{period}),/IMAGE_ROBUSTNESS_NOISE_PERIOD/);
});
test("new degradation primitives preserve source pixels",()=>{
  const source=Object.freeze(image.map(row=>Object.freeze(row.slice())));
  for(const operation of [{method:"intensity-linear-v1",gain:0.9,offset:15},{method:"box-blur-v1",radius:1},{method:"occlude-rectangle-v1",x:0,y:0,width:1,height:1}]){
    const out=applyImageDegradation(source,operation);
    assert.notStrictEqual(out,source);
    assert.ok(out.every((row,i)=>row!==source[i]&&row.every(v=>Number.isInteger(v)&&v>=0&&v<=255)));
    assert.deepStrictEqual(source,image);
  }
});

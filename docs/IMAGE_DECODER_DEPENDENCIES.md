...
Strict analyses additionally retain a full configuration snapshot, the eight compatibility implementation hashes and an `ekg-image-analysis-execution-v1` identity covering the privately loaded transitive JS/JSON artifacts. `lib/image_analysis_execution.js` hashes the same bounded UTF-8 buffers it compiles/parses, rather than hashing disk bytes after execution through a potentially stale ambient CommonJS cache. All captured artifacts must still match before/after generation and during persistence/normal reopening. The digitization support module is loaded and identified, but this does not claim that an existing extraction was produced by that loaded module. The trusted loader, Node runtime, built-ins and native binaries are outside this artifact graph; retain exact repository commit/tree, decoder environment and acceptance receipts separately. This is not actor authentication or complete deployment attestation.

Persistence binds both canonical and supplemental analysis quality back to the exact extraction and rechecks strict quality acceptance before publication/reopening. Old strict records without the new identity, and records from a different implementation version, cannot pass current strict reopening. `auditImageAnalysis` in `lib/image_analysis_store.js` is a separate read-only path that verifies stored canonical bytes, content identities and bound case/extraction integrity without loading the analysis implementation or rerunning measurements. Its wrapper explicitly reports implementation provenance `NOT_VERIFIED`, semantic validation `NOT_PERFORMED`, and `replayPerformed: false`; it does not admit the artifact for new persistence. Historical bytes and hashes are never rewritten or presented as matching new source. Non-strict legacy records retain their existing validation behavior. Hashes detect byte drift, not a privileged writer constructing a new internally consistent artifact.
